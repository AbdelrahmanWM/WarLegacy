...
bg2 = pygame.image.load("Objects\\bg3.png").convert_alpha()
    ...
    def hit(self,x,position):
            ...
            if x==0:
                if self.right and self.position[0]>position or self.left and self.position[0]<position:
                    ...
                    self.position[0] = self.position[0]-200*self.direction
                    self.canmove=1  
                    self.step=7
                    self.step2=-7
                else:
...
class AlienBullet(Bullet):
    ...
    def draw(self,screen):
        ...
        if newcooldown-self.cooldown>=choice([300,400,500,200,100,300,300,300,50]):
          self.trace=0  
        ...
        self.vec.from_polar((choice([15,13,12,10]), self.sX))
        rotated_image = pygame.transform.rotozoom(self.image, -self.sX,.5)
        new_rect = rotated_image.get_rect(center = self.image.get_rect(topleft = (self.position[0]-35,self.position[1]-32)).center)
        if self.view==1:
                if self.destroyed==0:
